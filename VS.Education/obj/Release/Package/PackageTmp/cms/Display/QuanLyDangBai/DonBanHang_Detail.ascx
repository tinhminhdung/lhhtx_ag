                    ...
                    <table id="order_details" class="table table-cart scollldh">
                        <thead class="thead-default">
                            <tr>
                                ...
                                <th style="border-bottom: none;">Trạng thái Người Mua</th>
                                <th style="border-bottom: none;">Trạng thái Nhà CC</th>
                                ...
                                <ItemTemplate>
                                    <tr style="height: 50px" class="tr_while" onmouseover="this.className='tr_while_over'" onmouseout="this.className='tr_while'">
                                        ...
                                        <td style="padding-left: 5px; text-align: center !important" class="cartadmin">
                                            <%#ShowtrangthaiNMH(DataBinder.Eval(Container.DataItem, "TrangThaiNguoiMuaHang").ToString(),DataBinder.Eval(Container.DataItem, "LyDoTraHang").ToString())%>
                                        </td>
                                        <td style="padding-left: 5px; text-align: center !important" class="cartadmin">
                                            <div class="TTDuyetdon">
                                                <%#Showtrangthai(DataBinder.Eval(Container.DataItem, "TrangThaiNhaCungCap").ToString())%><br />
                                                <button type="button" class="btn btn-primary" style='<%#EnableLock_DuyetHangstyle(Eval("TrangThaiNhaCungCap").ToString())%>; background: #fe3232; color: #fff; margin: auto; margin-bottom: 9px; width: 100px;' data-toggle="modal" onclick="GanGiaTri('<%#DataBinder.Eval(Container.DataItem,"ID")%>')" data-target="#exampleModal">Duyệt đơn</button>
                                                <%-- <asp:LinkButton Style='float: right; width: 172px !important' class="Duyetdon" ID="lnkDuyetdon" CommandName="DuyetDon" CommandArgument='<%#DataBinder.Eval(Container.DataItem,"ID")%>' runat="server">Duyệt đơn</asp:LinkButton>
                                                --%>
                                                 <asp:LinkButton Visible='<%#EnableLock_DuyetHang(Eval("TrangThaiNhaCungCap").ToString())%>' ID="lnkHuydon" OnLoad="HuyDon_Load" CommandName="HuyDon" CssClass="huydon" CommandArgument='<%#DataBinder.Eval(Container.DataItem,"ID")%>' runat="server">Hủy đơn</asp:LinkButton>
                                                <asp:LinkButton ID="LinkButton1" Visible='<%#EnableLock_TraHang(Eval("TrangThaiNhaCungCap").ToString())%>' OnLoad="HoanTien_Load" CommandName="HoanTien" CssClass="HoanTien" CommandArgument='<%#DataBinder.Eval(Container.DataItem,"ID")%>' runat="server">Hoàn lại tiền</asp:LinkButton>
                                                <asp:LinkButton Visible='<%#EnableLock_TraHang(Eval("TrangThaiNhaCungCap").ToString())%>' ID="LinkButton2" OnLoad="KhieuKien_Load" CommandName="KhieuKien" CssClass="huydon" CommandArgument='<%#DataBinder.Eval(Container.DataItem,"ID")%>' runat="server">Khiếu Kiện</asp:LinkButton>
                                                <asp:TextBox ID="txtLyDoHuyHang" Visible='<%#EnableLock_HuyDonHang(Eval("TrangThaiNhaCungCap").ToString())%>' Text='<%#DataBinder.Eval(Container.DataItem, "LyDoHuyHang")%>' placeholder="Ghi rõ lý do Hủy hàng" Style="border: 1px solid #a9a9a9; border-radius: 3px; text-align: left; width: 200px; height: 65px; margin-top: 10px;" TextMode="MultiLine" CssClass="txt_css" Width="40px" runat="server" AutoPostBack="true" OnTextChanged="txtLyDoHuyHang_TextChanged"></asp:TextBox>
                                                <!-- Modal -->
                                            </div>
                                        </td>
                                        ...
                                        <td style="padding-left: 5px; text-align: center !important" class="cartadmin"><%#(Eval("ThanhToanNCC").ToString())%></td>
